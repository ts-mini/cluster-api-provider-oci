...
keys = ["OCI_TENANCY_ID", "OCI_USER_ID", "OCI_CREDENTIALS_FINGERPRINT",
        "OCI_REGION", "OCI_CREDENTIALS_KEY_PATH"]
...
def validate_auth():
    ...
    for sub in substitutions:
        if sub[-4:] == "_B64":
            ...
            print("{} was not specified in tilt-settings.json, attempting to load {}".format(
                base64_decode(os.environ[sub]), sub))
...
def patch_args_with_extra_args(namespace, name, extra_args):
    args_str = str(local(
        "{} get deployments {} -n {} -o jsonpath={{.spec.template.spec.containers[1].args}}".format(kubectl_cmd, name, namespace)))
    ...
    if args_to_add:
        ...
        local("{} patch deployment {} -n {} --type json -p='{}'".format(kubectl_cmd,
...
def capoci():
    ...
    if settings.get("extra_args"):
        ...
        if oci_extra_args:
            ...
            append_arg_for_container_in_deployment(
                yaml_dict, "capoci-controller-manager", "capoci-system", "cluster-api-oci-controller", oci_extra_args)
    ...
    local_resource(
        ...
        deps=["api", "cloud", "config", "controllers", "exp", "feature",
              "pkg", "go.mod", "go.sum", "main.go", "auth-config.yaml"],
        labels=["cluster-api"],
    )
    ...
    docker_build(
        ref="tsmini/cluster-api-oci-controller-amd64:dev",
        context="./.tiltbuild/",
...
def base64_decode(to_decode):
    decode_blob = local(
        "echo '{}' | base64 --decode -".format(to_decode), quiet=True, echo_off=True)
